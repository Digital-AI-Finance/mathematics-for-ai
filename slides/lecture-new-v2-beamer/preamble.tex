...
% Fallback: If Inter not available, uncomment:
...
\definecolor{BGCard}{HTML}{1E3044}
...
\newenvironment{formulabox}{%
  \begin{tcolorbox}[
    colback=BGDarker, colframe=PillarBlue!30,
    boxrule=1pt, arc=6pt,
    halign=center,
...
\newcommand{\sectiondivider}[6]{%
  \begin{frame}[plain]
    ...
    \vspace{1em}
    {\color{PillarBlue}\rule{60pt}{3pt}}
    \vfill
    \note{#6}
  \end{frame}
}
...
\newcommand{\statnumber}[3][PillarBlue]{%
  ...
  {\scriptsize\scshape\color{TextSecondary}\textls[60]{#3}}}
...
\newcommand{\pillaritem}[3]{%
  \begin{tcolorbox}[colback=BGCard, colframe=BGCard,
    ...
    on line, width=0.18\textwidth]
    {\color{#1}\rule{6pt}{6pt}}\hspace{0.3em}%
    {\small\bfseries\color{#1}#2}\hfill%
    {\scriptsize\color{TextSecondary}#3}
  \end{tcolorbox}}
